...
public class DiskBasedCache implements Cache {
    ...
    public synchronized void clear() {
        ...
        this.mEntries.clear();
        this.mTotalSize = 0;
        VolleyLog.m613d("Cache cleared.", new Object[0]);
    }
            ...
            if (entry2 != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        cis = cis2;
                        try {
                            VolleyLog.m613d("%s: %s", file.getAbsolutePath(), e.toString());
                            remove(key);
                            if (cis != null) {
                                try {
                                    cis.close();
    ...
    public synchronized void initialize() {
        ...
        if (this.mRootDirectory.exists()) {
            ...
            VolleyLog.m614e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
        }
        return;
        if (fis != null) {
            try {
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (e.writeHeader(fos)) {
                ...
                fos.close();
                VolleyLog.m613d("Failed to write header for %s", file.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e2) {
            if (!file.delete()) {
                VolleyLog.m613d("Could not clean up file %s", file.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            VolleyLog.m613d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        if (this.mTotalSize + ((long) neededSpace) >= ((long) this.mMaxCacheSizeInBytes)) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m616v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (iterator.hasNext()) {
                ...
                if (getFileForKey(e.key).delete()) {
                    ...
                    VolleyLog.m613d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
                }
                iterator.remove();
                prunedFiles++;
            ...
            if (VolleyLog.DEBUG) {
                VolleyLog.m616v("pruned %d files, %d bytes, %d ms", Integer.valueOf(prunedFiles), Long.valueOf(this.mTotalSize - before), Long.valueOf(SystemClock.elapsedRealtime() - startTime));
            }
        }
    }
    ...
    static void writeString(OutputStream os, String s) throws IOException {
        byte[] b = s.getBytes(HttpURLConnectionBuilder.DEFAULT_CHARSET);
        writeLong(os, (long) b.length);
        os.write(b, 0, b.length);
    }

    static String readString(InputStream is) throws IOException {
        return new String(streamToBytes(is, (int) readLong(is)), HttpURLConnectionBuilder.DEFAULT_CHARSET);
    }
