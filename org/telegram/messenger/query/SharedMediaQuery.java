...
import org.telegram.tgnet.TLRPC.TL_inputMessagesFilterAudio;
...
public class SharedMediaQuery {
    ...
    public static void loadMedia(long uid, int offset, int count, int max_id, int type, boolean fromCache, int classGuid) {
        ...
        if (type == 0) {
            ...
            req.filter = new TL_inputMessagesFilterAudio();
        } else if (type == 3) {
    ...
    public static boolean canAddMessageToMedia(Message message) {
        ...
        if ((message.media instanceof TL_messageMediaPhoto) || (message.media instanceof TL_messageMediaVideo) || (message.media instanceof TL_messageMediaDocument) || (message.media instanceof TL_messageMediaAudio)) {
            return true;
        }
        if (message.entities.isEmpty()) {
            return false;
        ...
        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    while (cursor.next()) {
                        AbstractSerializedData nativeByteBuffer = new NativeByteBuffer(cursor.byteArrayLength(0));
                        if (!(nativeByteBuffer == null || cursor.byteBufferValue(0, (NativeByteBuffer) nativeByteBuffer) == 0)) {
